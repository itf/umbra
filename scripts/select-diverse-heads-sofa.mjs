 ...
 * from SADIE II (H4..H20) and HUTUBS (pp1..96) SOFA files.
 *
 ...
 *
 * After baking the selected heads (scripts/bake-hrtf.mjs, + resample-hrtf.mjs for 44.1k
 * sources like HUTUBS), ALWAYS run `node scripts/normalize-base-heads.mjs` so every
 * candidate is scalar-level-matched to sadie_h3 — otherwise the head A/B changes VOLUME,
 * not just spatial cues, and some raw heads clip past 1.0.
 *
 * Usage: node scripts/select-diverse-heads-sofa.mjs <sadie_dir> <hutubs_dir> [N]
 */
import { ready, FS, File } from 'h5wasm';
import { readFileSync, readdirSync, existsSync } from 'node:fs';
import { join } from 'node:path';

const [, , sadieDir, hutubsDir, nArg] = process.argv;
if (!sadieDir || !hutubsDir) {
  console.error('usage: node scripts/select-diverse-heads-sofa.mjs <sadie_dir> <hutubs_dir> [N]');
  process.exit(1);
}
...
for (const fn of readdirSync(hutubsDir).filter((d) => /^pp\d+\.sofa$/.test(d)).sort((a, b) => parseInt(a.slice(2), 10) - parseInt(b.slice(2), 10))) {
  const num = fn.slice(2).replace('.sofa', '');
  cands.push({ id: `hutubs_pp${num}`, label: `HUTUBS ${num}`, path: join(hutubsDir, fn) });
}
console.log(`Candidates: ${cands.length} (SADIE + HUTUBS). Extracting features on ${NDIR} common dirs…`);
